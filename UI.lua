...
-- i did not make this ui library, i found it here (I only applied edits)
-- https://raw.githubusercontent.com/bloodball/-back-ups-for-libs/main/wave
local CoreGui=game:service'CoreGui'

if CoreGui:FindFirstChild('PScreen') then CoreGui.PScreen:Destroy() end
if shared.LibraryConnectionsLib then
	shared.LibraryConnectionsLib:Disconnect()
end
...
function library:Main()
	...
	shared.LibraryConnectionsLib=game:service'UserInputService'.InputEnded:Connect(function(Input,GPE)
		if not GPE then
			if Input.KeyCode==Enum.KeyCode.P then
				GUIEnabled=not GUIEnabled
				PanelContainer.Visible=GUIEnabled
			end
		end
	end)
    ...
    function main:Tab(name)
        ...
        TabButton.MouseButton1Down:Connect(function()
            for i, v in ipairs(TabContainer:GetChildren()) do
                if v:IsA('TextButton') then
                    tween:Create(v, TweenInfo.new(0.2), {TextStrokeTransparency = 1}):Play()
                end
            end
            for i, v in ipairs(folder1:GetChildren()) do
                v.Visible = false
            end
            ...
            function section:Item(type, name, callback, options)
                ...
                elseif type:lower() == 'slider' then
                    ...
                    SliderButton.MouseButton1Down:Connect(function()
						MoveConnection = game:service('RunService').Heartbeat:Connect(function()
                            tween:Create(t, TweenInfo.new(0.1), {TextTransparency = 0}):Play()
							local Scale = math.clamp(game:service('Players').LocalPlayer:GetMouse().X - SliderButton.AbsolutePosition.X, 0, SliderButton.AbsoluteSize.X) /  SliderButton.AbsoluteSize.X
						...
						game:service('UserInputService').InputEnded:Connect(function(Check)
